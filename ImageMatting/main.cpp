...
void onMouse( int event, int x, int y, int, void* param)
{
	...
	if (event == EVENT_LBUTTONDOWN || bLeftButton)
    {
        // Foreground
        cv::circle(trimap, cv::Point(x,y), circleSize, Scalar(255,255,255), -1);
        bLeftButton = true;
    }
	else if (event == EVENT_RBUTTONDOWN || bRightButton)
    {
        // Background
        cv::circle(trimap, cv::Point(x,y), circleSize, Scalar(0,0,0), -1);
        bRightButton = true;
    }
    else
    {
